...
bool DLL candle_list_scan(candle_list_handle *list);
bool DLL candle_list_free(candle_list_handle list);
bool DLL candle_list_length(candle_list_handle list, uint8_t *len);

bool DLL candle_dev_get(candle_list_handle list, uint8_t dev_num, candle_handle *hdev);
bool DLL candle_dev_get_state(candle_handle hdev, candle_devstate_t *state);
#ifdef UNICODE
__declspec(dllexport) wchar_t * __cdecl candle_dev_get_path(candle_handle hdev);
#else
...
bool DLL candle_dev_free(candle_handle hdev);

bool DLL candle_channel_count(candle_handle hdev, uint8_t *num_channels);
bool DLL candle_channel_get_capabilities(candle_handle hdev, uint8_t ch, candle_capability_t *cap);
bool DLL candle_channel_set_timing(candle_handle hdev, uint8_t ch, candle_bittiming_t *data);
bool DLL candle_channel_set_bitrate(candle_handle hdev, uint8_t ch, uint32_t bitrate);
bool DLL candle_channel_start(candle_handle hdev, uint8_t ch, uint32_t flags);
bool DLL candle_channel_stop(candle_handle hdev, uint8_t ch);

bool DLL candle_frame_send(candle_handle hdev, uint8_t ch, candle_frame_t *frame);
bool DLL candle_frame_read(candle_handle hdev, candle_frame_t *frame, uint32_t timeout_ms);

candle_frametype_t DLL candle_frame_type(candle_frame_t *frame);
uint32_t DLL candle_frame_id(candle_frame_t *frame);
bool DLL candle_frame_is_extended_id(candle_frame_t *frame);
bool DLL candle_frame_is_rtr(candle_frame_t *frame);
uint8_t DLL candle_frame_dlc(candle_frame_t *frame);
__declspec(dllexport) uint8_t* __cdecl candle_frame_data(candle_frame_t *frame);
uint32_t DLL candle_frame_timestamp_us(candle_frame_t *frame);

candle_err_t DLL candle_dev_last_error(candle_handle hdev);
candle_err_t DLL candle_list_last_error(candle_list_handle list);
