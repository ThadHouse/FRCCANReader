...
static bool candle_read_device_name(HDEVINFO hdi, SP_DEVINFO_DATA* spDevInfoData, candle_device_t* dev)
{
	...
	if (SetupDiGetDeviceRegistryProperty(hdi, spDevInfoData,
		SPDRP_FRIENDLYNAME, &DataT, (PBYTE)dev->name, sizeof(dev->name) / sizeof(dev->name[0]), &nSize)) {
		return true;
	}
	else {
#ifdef UNICODE
		swprintf(dev->name, sizeof(dev->name) / sizeof(dev->name[0]), L"Error: Failed to read name");
#else
        snprintf(dev->name, sizeof(dev->name) / sizeof(dev->name[0]), "Error: Failed to read name");
#endif
		return false;
	}
}
...
static bool candle_read_di(HDEVINFO hdi, SP_DEVICE_INTERFACE_DATA interfaceData, candle_device_t *dev)
{
    ...
    if (!SetupDiGetDeviceInterfaceDetail(hdi, &interfaceData, detail_data, length, &requiredLength, NULL) ) {
        ...
        retval = false;
    } else if (FAILED(StringCchCopy(dev->path, sizeof(dev->path) / sizeof(dev->path[0]), detail_data->DevicePath))) {
...
bool __cdecl candle_list_scan(candle_list_handle *list)
{
    if (list==NULL) {
        return false;
    }
...
#ifdef UNICODE
			swprintf(l->dev[i].name, sizeof(l->dev[i].name) / sizeof(l->dev[i].name[0]), L"Unknown");
#else
            snprintf(l->dev[i].name, sizeof(l->dev[i].name) / sizeof(l->dev[i].name[0]), "Unknown");
#endif
		}
...
candle_err_t __cdecl DLL candle_list_last_error(candle_list_handle list)
{
	candle_list_t* dev = (candle_list_t*)list;
	return dev->last_error;
}

bool __cdecl DLL candle_list_free(candle_list_handle list)
{
    free(list);
    return true;
}

bool __cdecl DLL candle_list_length(candle_list_handle list, uint8_t *len)
{
...
bool __cdecl DLL candle_dev_get(candle_list_handle list, uint8_t dev_num, candle_handle *hdev)
{
    candle_list_t *l = (candle_list_t *)list;
    if (l==NULL) {
        return false;
...
bool __cdecl DLL candle_dev_get_state(candle_handle hdev, candle_devstate_t *state)
{
    if (hdev==NULL) {
        return false;
    } else {
...
#ifdef UNICODE
wchar_t* __cdecl DLL candle_dev_get_path(candle_handle hdev)
#else
char* __cdecl DLL candle_dev_get_path(candle_handle hdev)
#endif
{
    if (hdev==NULL) {
        return NULL;
...
#ifdef UNICODE
wchar_t* __cdecl DLL candle_dev_get_name(candle_handle hdev)
#else
char* __cdecl DLL candle_dev_get_name(candle_handle hdev)
#endif
{
	if (hdev == NULL) {
		return NULL;
...
bool __cdecl DLL candle_dev_open(candle_handle hdev)
{
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_dev_get_timestamp_us(candle_handle hdev, uint32_t *timestamp_us)
{
	return candle_ctrl_get_timestamp(hdev, timestamp_us);
}

bool __cdecl DLL candle_dev_close(candle_handle hdev)
{
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_dev_free(candle_handle hdev)
{
    free(hdev);
    return true;
}

candle_err_t __cdecl DLL candle_dev_last_error(candle_handle hdev)
{
    candle_device_t *dev = (candle_device_t*)hdev;
    return dev->last_error;
}

bool __cdecl DLL candle_channel_count(candle_handle hdev, uint8_t *num_channels)
{
...
bool __cdecl DLL candle_channel_get_capabilities(candle_handle hdev, uint8_t ch, candle_capability_t *cap)
{
    // TODO check if info was already read from device; try to do so; throw error...
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_channel_set_timing(candle_handle hdev, uint8_t ch, candle_bittiming_t *data)
{
...
bool __cdecl DLL candle_channel_set_bitrate(candle_handle hdev, uint8_t ch, uint32_t bitrate)
{
    // TODO ensure device is open, check channel count..
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_channel_start(candle_handle hdev, uint8_t ch, uint32_t flags)
{
    // TODO ensure device is open, check channel count..
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_channel_stop(candle_handle hdev, uint8_t ch)
{
    // TODO ensure device is open, check channel count..
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_frame_send(candle_handle hdev, uint8_t ch, candle_frame_t *frame)
{
    // TODO ensure device is open, check channel count..
    candle_device_t *dev = (candle_device_t*)hdev;
...
bool __cdecl DLL candle_frame_read(candle_handle hdev, candle_frame_t *frame, uint32_t timeout_ms)
{
...
candle_frametype_t __cdecl DLL candle_frame_type(candle_frame_t *frame)
{
...
uint32_t __cdecl DLL candle_frame_id(candle_frame_t *frame)
{
    return frame->can_id & 0x1FFFFFFF;
}

bool __cdecl DLL candle_frame_is_extended_id(candle_frame_t *frame)
{
    return (frame->can_id & CANDLE_ID_EXTENDED) != 0;
}

bool __cdecl DLL candle_frame_is_rtr(candle_frame_t *frame)
{
    return (frame->can_id & CANDLE_ID_RTR) != 0;
}

uint8_t __cdecl DLL candle_frame_dlc(candle_frame_t *frame)
{
    return frame->can_dlc;
}

uint8_t* __cdecl DLL candle_frame_data(candle_frame_t *frame)
{
    return frame->data;
}

uint32_t __cdecl DLL candle_frame_timestamp_us(candle_frame_t *frame)
{
